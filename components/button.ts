...
export const buttonOptionStyles = generateOptionsStyles();
    ...
    render(ctx: ICtx, me: IElementBobrilNode) {
        ...
        b.style(
            ...
            ctx.data.variant !== ButtonVariant.Navbar &&
                ctx.data.variant !== ButtonVariant.DropdownNav &&
                ctx.data.variant !== ButtonVariant.ListGroup &&
                buttonOptionStyles(ctx.data.option || ButtonOption.Default)
        );
        ...
        me.attrs[typeAttr] = ((ctx.data.type && ButtonType[ctx.data.type]) || me.attrs[typeAttr] || ButtonType[ButtonType.Button])
            .toString()
            .toLowerCase();
...
function generateOptionsStyles(): IDictionary<ButtonOption, b.IBobrilStyle> {
    ...
    Object.keys(ButtonOption).forEach(key => {
        ...
        if (!isNaN(castedValue)) {
            result(
                castedValue,
                castedValue === ButtonOption.Close ? helpers.close : b.styleDef(`btn-${ButtonOption[castedValue].toLowerCase()}`)
            );
        }
    });
