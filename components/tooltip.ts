...
function isManualTrigger(ctx: ITooltipCtx): boolean {
    return ctx.data.trigger !== undefined && ctx.data.trigger.length === 1 && ctx.data.trigger[0] === TooltipTrigger.Manual;
}

function unregister(ctx: ITooltipCtx): void {
    if (ctx.tooltipedElement) {
