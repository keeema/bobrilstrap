...
export const Alert = b.createDerivedComponent<IAlertData, IBaseData>(Elem, {
    ...
    render(ctx: IAlertCtx, me: IElementBobrilNode) {
        ...
        if (ctx.data.dismissButton) {
            b.style(me, alertStyles.alertDismissable, !!animation && alertStyles.fade, !!animation && alertStyles.in);
    ...
    postInitDom(ctx: IAlertCtx, _me: b.IBobrilNode, element: HTMLElement) {
        ...
        if (ctx.data.timeout) {
            ctx.timeoutId = setTimeout(() => dismissOnTimeout(ctx, element), ctx.data.timeout);
        }
    },
...
function dismissOnTimeout(ctx: IAlertCtx, element: HTMLElement): void {
    if (ctx.timeoutId) {
        clearTimeout(ctx.timeoutId);
        ctx.timeoutId = 0;
    }
